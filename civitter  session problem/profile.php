...
if (array_key_exists('kullanici', $_SESSION)){
	...
	foreach ($users as $user) {
		...
		echo '<a href="editprofile.php?id='.$user['User_Id'].'"><h4>'.$user['User_Name'].'</h4></a>';
		echo '<p id="photoToggle">Upload Photo</p>';
		echo '<form action="photoupload.php?id='.$user['User_Id'].'" id="form-photo" >';

		echo '<input type="file" name="photo" id="photo"><input type="submit" id="photosubmit"></form>';
		echo '<p  class="yukle">Yükle</p>';
		
		}
		else {
			echo '<h4>'.$user['User_Name'].'</h4>';
			if ($unfollow==0){
			echo '<a class="follow" href="follow.php?id='.$user['User_Id'].'">';
			echo '<i class="glyphicon glyphicon-plus" style="color:white;">Follow</i></a>';
			}
...
$row=$count[0];
	...
	echo '<p class="postcount">Civits : '.$row.'</p></div>';
...
if($posts->rowCount()){
	foreach ($posts as $post) {
		...
		echo '<p class="userpost">'.$post['Civit'].'</p><br>';
